...
function MainApp({currentUser,onLogout}){
  ...
  const triggerGasSync = async () => {
    const WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbyGCr_9jehD-ktTd34eRLaOvdpBkdQmnki4WDFJwqYx4pvjyzRSDvRWWPDMGgIkbZD3ZQ/exec';
  ...
  const dlXLS=()=>{
    ...
    chronologicalTxs.forEach((t, index) => {
      ...
      rows.push([
        ...
        t.masuk > 0 ? formulaCell(`C${currentRow}*E${currentRow}`) : numCell(0),
        t.keluar > 0 ? formulaCell(`C${currentRow}*E${currentRow}`) : numCell(0),
