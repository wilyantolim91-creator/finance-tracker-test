...
/* ─────────────────── METRICS ─────────────────── */
function metrics(p) {
  const txs = [...p.transactions].sort((a,b)=>a.tgl.localeCompare(b.tgl));
  const dpMasuk = txs.filter(t=>t.kategori==="Transfer"&&t.masuk>0).reduce((s,t)=>s+t.masuk,0);
  const perKas = Object.fromEntries(KAS_LIST.map(k=>[k,0]));
  txs.forEach(t=>{ perKas[t.kas] = (perKas[t.kas]||0) + t.masuk - t.keluar; });
  const perKat = {};
...
/* ─────────────────── UI ATOMS ─────────────────── */
const CAT_CLS = {
  Material:"text-amber-300 bg-amber-400/10 ring-amber-400/20",
  Upah:"text-emerald-300 bg-emerald-400/10 ring-emerald-400/20",
  Labor:"text-teal-300 bg-teal-400/10 ring-teal-400/20",
  Transfer:"text-sky-300 bg-sky-400/10 ring-sky-400/20",
  "Transfer Internal":"text-violet-300 bg-violet-400/10 ring-violet-400/20",
  Lainnya:"text-slate-300 bg-slate-400/10 ring-slate-400/20",
};

function Card({label,value,sub,icon:I,accent}) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/5 bg-gradient-to-b from-white/[0.06] to-transparent p-5">
      <div className={`absolute -right-6 -top-6 h-24 w-24 rounded-full blur-2xl ${accent}`}/>
      <div className="flex items-center gap-2 text-slate-400"><I className="h-4 w-4"/><span className="text-xs font-medium uppercase tracking-wider">{label}</span></div>
...
function FInput({label,v,onChange,type="text",opts}) {
  const cls = "w-full rounded-lg border border-white/10 bg-slate-800 px-2 py-1.5 text-white placeholder-slate-500 outline-none focus:border-cyan-400/40";
  return (
    <label className="block">
      <span className="mb-1 block text-[10px] uppercase tracking-wider text-slate-500">{label}</span>
      {opts
        ? <select value={v} onChange={e=>onChange(e.target.value)} className={cls}>{opts.map(o=><option key={o} className="bg-slate-800">{o}</option>)}</select>
...
function LoginScreen({users,onLogin}) {
  ...
  const go = () => {
    if(!u||!p){setErr("Lengkapi username & password.");return;}
    if(users[u]?.password===p) onLogin({username:u,...users[u]});
    else setErr("Username atau password salah.");
  };
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 px-4" style={{fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900/80 p-8 backdrop-blur">
        <div className="mb-8 text-center">
          ...
          <div className="flex items-center justify-center gap-2">
            <h1 className="text-2xl font-bold text-white" style={{fontFamily:"'Space Grotesk'"}}>FinTrack</h1>
            <span className="rounded-full bg-cyan-400/20 px-2 py-0.5 text-[10px] font-bold text-cyan-300 ring-1 ring-cyan-400/30">V1</span>
          </div>
          <p className="mt-1 text-xs text-slate-500">By <span className="text-slate-400 font-medium">Mrlims</span> · Creator</p>
        </div>
        <div className="space-y-4">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-300">Username</label>
            <input value={u} onChange={e=>setU(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Masukkan username"
              className="w-full rounded-xl border border-white/10 bg-white px-4 py-2.5 text-slate-900 placeholder-slate-400 outline-none focus:border-cyan-400"/>
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-300">Password</label>
            <input type="password" value={p} onChange={e=>setP(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Masukkan password"
              className="w-full rounded-xl border border-white/10 bg-white px-4 py-2.5 text-slate-900 placeholder-slate-400 outline-none focus:border-cyan-400"/>
          </div>
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-6 shadow-2xl" onClick={e=>e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input value={f.username} disabled={!!init} onChange={e=>setF({...f,username:e.target.value})} placeholder="username"
              className="w-full rounded-lg border border-white/10 bg-slate-800 px-3 py-2 text-white placeholder-slate-500 outline-none disabled:opacity-50"/>
          </div>
          <div>
            <label className="mb-1 block text-[10px] uppercase tracking-wider text-slate-500">Password{init?" (kosong = tidak diubah)":""}</label>
            <input type="password" value={f.password} onChange={e=>setF({...f,password:e.target.value})} placeholder="••••••••"
              className="w-full rounded-lg border border-white/10 bg-slate-800 px-3 py-2 text-white placeholder-slate-500 outline-none"/>
          </div>
          {!isSelf && (
            <div>
              <label className="mb-1 block text-[10px] uppercase tracking-wider text-slate-500">Role</label>
              <select value={f.role} onChange={e=>setF({...f,role:e.target.value})} className="w-full rounded-lg border border-white/10 bg-slate-800 px-3 py-2 text-white outline-none">
        ...
        <div className="mt-5 flex gap-3">
          ...
          <button onClick={()=>{onSave(f);onClose();}} className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-emerald-500 py-2.5 text-sm font-semibold text-slate-950 hover:bg-emerald-400">
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-white/5 bg-white/[0.03] p-5">
        <div className="mb-3 flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">Akun Saya</span>
        </div>
        <div className="flex items-center justify-between">
          ...
          <button onClick={()=>setEditUser(currentUser.username)} className="flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-1.5 text-xs text-slate-300 hover:bg-white/10">
      ...
      <div className="rounded-2xl border border-white/5 bg-white/[0.03] p-5">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-white">Manajemen User</span>
          </div>
          <button onClick={()=>setAddOpen(true)} className="flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-sky-400 to-cyan-500 px-3 py-1.5 text-xs font-semibold text-slate-950">
            <Plus className="h-3.5 w-3.5"/> Tambah Staff
          </button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {Object.entries(users).map(([uname,ud])=>(
                <tr key={uname} className="border-t border-white/5">
                  <td className="py-3 pr-4">
                    <div className="flex items-center gap-2">
                      <div className="grid h-7 w-7 place-items-center rounded-full bg-cyan-400/10 text-xs font-bold text-cyan-300">{uname[0].toUpperCase()}</div>
                      <span className="font-medium text-white">{uname}</span>
                      {uname===currentUser.username && <span className="rounded-full bg-white/10 px-1.5 py-0.5 text-[9px] text-slate-400">Saya</span>}
                    </div>
                  </td>
                  <td className="py-3 pr-4">
                    <span className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset ${ud.role==="admin"?"bg-violet-400/10 text-violet-300 ring-violet-400/20":"bg-emerald-400/10 text-emerald-300 ring-emerald-400/20"}`}>
                      {ud.role==="admin"?<ShieldCheck className="h-3 w-3"/>:<Eye className="h-3 w-3"/>}
                      {ud.role==="admin"?"Admin":"Staff"}
                    </span>
                  </td>
                  <td className="py-3 pr-4">
                  ...
                  <td className="py-3 text-center">
                    <div className="flex justify-center gap-1">
                      <button onClick={()=>setEditUser(uname)} className="rounded-lg p-1.5 text-slate-400 hover:bg-white/10 hover:text-cyan-300"><Edit2 className="h-3.5 w-3.5"/></button>
                      {uname!=="admin"&&uname!==currentUser.username&&(
                        <button onClick={()=>handleDel(uname)} className="rounded-lg p-1.5 text-slate-400 hover:bg-white/10 hover:text-rose-300"><Trash2 className="h-3.5 w-3.5"/></button>
                      )}
                    </div>
                  </td>
                </tr>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-6" onClick={e=>e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <FInput label="Tanggal" v={f.tgl} type="date" onChange={v=>setF({...f,tgl:v})}/>
          <FInput label="Kas" v={f.kas} opts={KAS_LIST} onChange={v=>setF({...f,kas:v})}/>
          <div className="col-span-2"><FInput label="Deskripsi" v={f.desc} onChange={v=>setF({...f,desc:v})}/></div>
          <FInput label="Kategori" v={f.kategori} opts={ALL_CATS} onChange={v=>setF({...f,kategori:v})}/>
          <FInput label="Tujuan" v={f.tujuan||""} onChange={v=>setF({...f,tujuan:v})}/>
          <FInput label="Masuk" v={f.masuk} type="number" onChange={v=>setF({...f,masuk:Number(v)||0})}/>
          <FInput label="Keluar" v={f.keluar} type="number" onChange={v=>setF({...f,keluar:Number(v)||0})}/>
        </div>
        <div className="mt-5 flex gap-3">
          <button onClick={onClose} className="flex-1 rounded-xl border border-white/10 py-2.5 text-sm font-medium text-slate-300 hover:bg-white/5">Batal</button>
          <button onClick={()=>{onSave(f);onClose();}} className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-emerald-500 py-2.5 text-sm font-semibold text-slate-950 hover:bg-emerald-400">
...
function FloatingAI({onAdd}) {
  ...
  const [parsed,setParsed]=useState(null); const [note,setNote]=useState("");
  const recRef=React.useRef(null);
  const reset=()=>{setParsed(null);setNote("");setStatus("");};
  const runText=async t=>{
    if(!t.trim())return;
    setStatus("thinking");setParsed(null);setNote("");
    try{setParsed(await parseAI(t));}catch{setNote("Gagal memproses.");}
    finally{setStatus("");setText("");}
  };
  const runImg=async file=>{
    if(!file)return;
    setStatus("thinking");setParsed(null);setNote("");
    try{const b64=await toB64(file);setParsed(await parseAI([{type:"image",source:{type:"base64",media_type:file.type||"image/jpeg",data:b64}},{type:"text",text:"Ekstrak transaksi dari nota ini."}]));}
    catch{setNote("Gagal membaca file.");}
    finally{setStatus("");}
  };
  const startVoice=()=>{
    const SR=window.SpeechRecognition||window.webkitSpeechRecognition;
    if(!SR){setNote("Browser belum support rekam suara.");return;}
    const rec=new SR();rec.lang="id-ID";rec.continuous=false;
    rec.onresult=e=>setText(Array.from(e.results).map(r=>r[0].transcript).join(""));
    rec.onend=()=>setStatus("");recRef.current=rec;
  ...
  return (
    <>
      {!open && (
        <button onClick={()=>setOpen(true)} className="fixed bottom-20 right-4 z-50 grid h-14 w-14 place-items-center rounded-full bg-gradient-to-br from-sky-400 to-cyan-500 text-slate-950 shadow-xl hover:scale-105">
          <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-cyan-400/40"/>
          <Sparkles className="relative h-6 w-6"/>
        </button>
      )}
      {open && (
        <div className="fixed bottom-20 right-4 z-50 flex w-[min(92vw,380px)] flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-900 shadow-2xl">
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
            <div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-cyan-400"/><span className="text-sm font-semibold text-white">Asisten AI</span></div>
            <button onClick={()=>{setOpen(false);reset();setText("");}} className="rounded-lg p-1 text-slate-400 hover:bg-white/10"><X className="h-5 w-5"/></button>
          </div>
            ...
            {parsed&&(
              <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <FInput label="Tujuan" v={parsed.tujuan} onChange={v=>setParsed({...parsed,tujuan:v})}/>
                  <FInput label="Masuk" v={parsed.masuk} type="number" onChange={v=>setParsed({...parsed,masuk:Number(v)||0})}/>
                  <FInput label="Keluar" v={parsed.keluar} type="number" onChange={v=>setParsed({...parsed,keluar:Number(v)||0})}/>
                </div>
                <div className="mt-3 flex gap-2">
                  ...
                  <button onClick={()=>{onAdd(parsed);reset();}} className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-emerald-500 py-2 text-sm font-semibold text-slate-950 hover:bg-emerald-400"><Check className="h-4 w-4"/> Simpan</button>
                </div>
              </div>
            )}
          </div>
          <div className="border-t border-white/10 p-3">
            ...
            <div className="flex items-end gap-2">
              <label className="grid h-9 w-9 shrink-0 cursor-pointer place-items-center rounded-lg border border-white/10 bg-white/[0.04] text-slate-300 hover:bg-white/10">
                <Camera className="h-4 w-4"/>
                <input type="file" accept="image/*" capture="environment" className="hidden" onChange={e=>runImg(e.target.files?.[0])}/>
              </label>
              <label className="grid h-9 w-9 shrink-0 cursor-pointer place-items-center rounded-lg border border-white/10 bg-white/[0.04] text-slate-300 hover:bg-white/10">
                <Paperclip className="h-4 w-4"/>
                <input type="file" accept="image/*,application/pdf" className="hidden" onChange={e=>runImg(e.target.files?.[0])}/>
              </label>
              ...
              <button onClick={()=>runText(text)} disabled={!text.trim()||status==="thinking"}
                className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-gradient-to-br from-sky-400 to-cyan-500 text-slate-950 disabled:opacity-40">
...
function MainApp({currentUser,setCurrentUser,users,setUsers,data,setData,onLogout}) {
  ...
  const dlPDF=async()=>{
    try{
      ...
      [[`Total Kontrak`,fmt(m.totalKontrak)],[`DP Masuk`,fmt(m.dpMasuk)],[`Sisa Pembayaran`,fmt(m.sisaPembayaran)],[`Total Pengeluaran`,fmt(m.totalBiaya)],[`Saldo Akhir`,fmt(m.saldoAkhir)]]
        .forEach(([l,v])=>{doc.text(`${l}: ${v}`,10,y);y+=5;});
      y+=3;doc.setFontSize(11);doc.text("Saldo per Kas",10,y);y+=6;doc.setFontSize(9);
      KAS_LIST.forEach(k=>{doc.text(`${k}: ${fmt(m.perKas[k])}`,10,y);y+=5;});
      y+=3;doc.setFontSize(11);doc.text("Pengeluaran per Kategori",10,y);y+=6;doc.setFontSize(9);
      Object.entries(m.perKat).forEach(([k,v])=>{doc.text(`${k}: ${fmt(v)}`,10,y);y+=5;});
  ...
  const navItems=[
    ...
    ...(isAdmin?[{id:"settings",label:"Pengaturan",icon:Settings}]:[]),
  ];
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 text-slate-200 antialiased" style={{fontFamily:"'Plus Jakarta Sans',ui-sans-serif,system-ui"}}>
      ...
      <div className="flex">
        <aside className="sticky top-0 hidden h-screen w-60 shrink-0 flex-col border-r border-white/5 bg-slate-950/80 px-4 py-6 lg:flex">
          <div className="mb-8 flex items-center gap-2 px-2">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-sm font-bold text-white" style={{fontFamily:"'Space Grotesk'"}}>FinTrack</span>
                <span className="rounded-full bg-cyan-400/20 px-1.5 py-0.5 text-[9px] font-bold text-cyan-300 ring-1 ring-cyan-400/30">V1</span>
              </div>
              <div className="text-[10px] text-slate-500">By <span className="text-slate-400">Mrlims</span> · Creator</div>
            </div>
          </div>
          <nav className="flex flex-col gap-1">
            {navItems.map(it=>(
              <button key={it.id} onClick={()=>setTab(it.id)}
                className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${tab===it.id?"bg-white/10 text-white":"text-slate-400 hover:bg-white/5 hover:text-slate-200"}`}>
                <it.icon className="h-4 w-4"/>{it.label}
              </button>
            ))}
          </nav>
          <div className="mt-auto rounded-xl border border-white/5 bg-white/[0.03] p-3 text-xs text-slate-400">
            User: <span className="text-cyan-300">{currentUser.username}</span>
            <div className="mt-1 text-[10px] capitalize">{isAdmin?"Akses semua project":currentUser.assignedProjects.join(", ")}</div>
          </div>
        </aside>
          ...
          <header className="sticky top-0 z-10 flex flex-wrap items-center justify-between gap-3 border-b border-white/5 bg-slate-950/80 px-5 py-4 backdrop-blur">
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <select value={proj} onChange={e=>setProject(e.target.value)}
                  className="appearance-none rounded-xl border border-white/10 bg-slate-800 py-2 pl-3 pr-9 text-sm font-semibold text-white outline-none">
                  {visProj.map(p=><option key={p} className="bg-slate-800 text-white">{p}</option>)}
                </select>
            ...
            {tab==="dashboard"&&(
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                  ...
                  <Card label="DP Masuk" value={fmtS(m.dpMasuk)} sub={`${((m.dpMasuk/m.totalKontrak)*100).toFixed(0)}% dari kontrak`} icon={ArrowDownLeft} accent="bg-emerald-500/30"/>
                ...
                <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                  <div className="rounded-2xl border border-white/5 bg-white/[0.03] p-5">
                    ...
                    <div className="space-y-2">
                      {KAS_LIST.map(k=>(
                        <button key={k} onClick={()=>{setKasFilter(k);setTab("transactions");}}
                          className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm text-slate-400 hover:bg-white/5 hover:text-white transition">
                          <span className="font-medium">{k}</span>
                          <span className={`font-semibold tabular-nums ${m.perKas[k]<0?"text-rose-400":"text-white"}`}>{fmt(m.perKas[k])}</span>
                        </button>
                      ))}
                      <div className="flex justify-between border-t border-white/5 pt-3 text-sm">
                        <span className="text-slate-400">Total</span>
                        <span className="font-semibold text-white">{fmt(m.saldoAkhir)}</span>
                      </div>
                    </div>
                  </div>
                  <div className="rounded-2xl border border-white/5 bg-white/[0.03] p-5">
                    ...
                    {Object.entries(m.perKat).map(([c,v])=>(
                      <div key={c} className="mb-2.5">
                        ...
                        <div className="h-1.5 overflow-hidden rounded-full bg-white/5">
                          <div className="h-full rounded-full bg-gradient-to-r from-sky-400 to-cyan-500" style={{width:`${(v/maxKat)*100}%`}}/>
                        </div>
                      </div>
                    ))}
                    <div className="mt-3 flex justify-between border-t border-white/5 pt-3 text-sm">
                      <span className="text-slate-300">Total Pengeluaran</span>
                      <span className="font-semibold text-white">{fmt(m.totalBiaya)}</span>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <div className="relative">
                      ...
                      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cari…"
                        className="w-40 rounded-lg border border-white/10 bg-slate-800 py-1.5 pl-8 pr-2 text-xs text-white placeholder-slate-500 outline-none"/>
                    </div>
                    <div className="relative">
                      <Filter className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-500"/>
                      <select value={kasFilter} onChange={e=>setKasFilter(e.target.value)}
                        className="appearance-none rounded-lg border border-white/10 bg-slate-800 py-1.5 pl-8 pr-7 text-xs text-white outline-none">
                        <option value="ALL" className="bg-slate-800">Semua Kas</option>
                        {KAS_LIST.map(k=><option key={k} className="bg-slate-800">{k}</option>)}
                      </select>
                    </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-white/10 bg-slate-950/95 backdrop-blur lg:hidden">
        ...
        <div className="border-t border-white/5 py-1 text-center text-[9px] text-slate-600">
          FinTrack <span className="text-cyan-800">V1</span> · By Mrlims
        </div>
      </nav>
    </div>
  );
